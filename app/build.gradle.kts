...
android {
    ...
    compileSdk = 36
    // The offline SDK snapshot available in this build environment only ships working
    // build-tools 36.0.0 (AGP 8.7's default 34.0.0 is not installed). Pin to the available
    // version so the build does not try to fetch/use the missing 34.0.0 package.
    ...
    defaultConfig {
        // Install identity (applicationId) is decoupled from the source namespace so multiple
        // builds can coexist on one device/TV — e.g. alongside the official "blbl.cat3399" build.
        // Override per-build with: -PappId=blbl.cat3399.v2 -PappName="BBLL 测试"
        applicationId = propOrEnv("appId") ?: "blbl.cat3399.feed"
        manifestPlaceholders["appName"] = propOrEnv("appName") ?: "XingBili"
        minSdk = 21
        targetSdk = 36
