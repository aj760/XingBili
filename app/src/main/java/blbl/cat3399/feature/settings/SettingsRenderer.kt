            ...
            "播放设置" ->
                listOf(
                    ...
                    SettingEntry(
                        ...
                        "播放器按钮文字提示（瞄准时显示名称）",
                        if (prefs.focusHintEnabled) "开" else "关",
                        "D-Pad 聚焦到播放器图标按钮时显示该按钮的名字",
                    ),
                    SettingEntry(
                        SettingId.WatchedBadgeUpMasterSpace,
                        "UP主空间显示「已看过」",
                        if (prefs.watchedBadgeUpMasterSpace) "开" else "关",
                        "在 UP 主空间视频列表中，为已播放完的视频显示「已看过」标记",
                    ),
                    SettingEntry(
                        SettingId.WatchedBadgeOtherLists,
                        "其他列表显示「已看过」",
                        if (prefs.watchedBadgeOtherLists) "开" else "关",
                        "在首页/分区/搜索/动态/我的等列表中，为已播放完的视频显示「已看过」标记",
                    ),
                    SettingEntry(
                        SettingId.ClearWatchedHistory,
                        "清除「已看过」记录",
                        "共 ${prefs.watchedBvids.size} 条",
                        "清除本地记录的已看过视频（不会删除 B 站观看历史）",
                    ),
                    SettingEntry(
