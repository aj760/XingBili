...
import blbl.cat3399.core.feedback.FeedbackSender
...
class SettingsInteractionHandler(
    ...
    fun onEntryClicked(entry: SettingEntry) {
        ...
        when (entry.id) {
            ...
            SettingId.FocusHintEnabled -> {
                prefs.focusHintEnabled = !prefs.focusHintEnabled
                AppToast.show(activity, "按钮文字提示：${if (prefs.focusHintEnabled) "开" else "关"}")
                renderer.refreshSection(entry.id)
            }

            SettingId.WatchedBadgeUpMasterSpace -> {
                prefs.watchedBadgeUpMasterSpace = !prefs.watchedBadgeUpMasterSpace
                AppToast.show(activity, "UP主空间「已看过」：${if (prefs.watchedBadgeUpMasterSpace) "开" else "关"}")
                renderer.refreshSection(entry.id)
            }

            SettingId.WatchedBadgeOtherLists -> {
                prefs.watchedBadgeOtherLists = !prefs.watchedBadgeOtherLists
                AppToast.show(activity, "其他列表「已看过」：${if (prefs.watchedBadgeOtherLists) "开" else "关"}")
                renderer.refreshSection(entry.id)
            }

            SettingId.ClearWatchedHistory -> {
                val count = prefs.watchedBvids.size
                if (count == 0) {
                    AppToast.show(activity, "没有可清除的「已看过」记录")
                    renderer.refreshSection(entry.id)
                ...
                AppPopup.confirm(
                    context = activity,
                    title = "清除「已看过」记录",
                    message = "确定清除本地记录的 $count 条「已看过」标记？\n（不会删除 B 站观看历史）",
                    positiveText = "清除",
                    negativeText = "取消",
                    cancelable = true,
                    onPositive = {
                        prefs.clearWatchedHistory()
                        AppToast.show(activity, "已清除「已看过」记录")
                        renderer.refreshSection(entry.id)
                    },
                    onNegative = { renderer.refreshSection(entry.id) },
                    onDismiss = { renderer.refreshSection(entry.id) },
                )
            }
            ...
            SettingId.Feedback -> {
                AppPopup.input(
                    ...
                    title = "意见反馈",
                    initial = "",
                    hint = "说说你的建议或遇到的问题（可附文字说明）",
                    minLines = 4,
                    positiveText = "提交",
                    validate = { it.isNotBlank() },
                    onPositive = { text ->
                        activity.lifecycleScope.launch {
                            val result = FeedbackSender.send(text)
                            withContext(Dispatchers.Main) {
                                AppToast.show(activity, result.message)
                            }
                        }
                    },
                )
