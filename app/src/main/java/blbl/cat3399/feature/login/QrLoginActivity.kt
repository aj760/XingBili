    ...
    private fun startFlow() {
        ...
        lifecycleScope.launch {
            try {
                ...
                // The auth_code API returns code != 0 on failure. Without this check a failed
                // response yields an empty url/auth_code and the QR never appears, leaving the poll
                // loop silently spinning forever against an empty auth_code.
                val genCode = gen.optInt("code", -1)
                if (genCode != 0) {
                    val genMsg = gen.optString("message", gen.optString("msg", "未知错误"))
                    throw IllegalStateException("申请二维码失败 code=$genCode msg=$genMsg")
                }
