...
internal fun PlayerActivity.handlePlaybackEnded(engine: BlblPlayerEngine) {
    ...
    // Locally record the video as "已看过" so the matching badge can appear on the UP master
    // space (and other video lists, per user setting). Cheap and idempotent.
    runCatching { BiliClient.prefs.markWatched(currentBvid) }
