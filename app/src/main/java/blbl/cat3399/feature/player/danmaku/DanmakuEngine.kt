...
internal class DanmakuEngine(
    ...
    override fun trimToTimeRange(minTimeMs: Long, maxTimeMs: Long) {
        ...
        if (start >= end) {
            // Window [min, max) is empty. Drop only the items strictly before `start` (already
            // passed) and keep everything from `start` onward — including any future danmaku —
            // instead of wiping the whole list. A bare items.clear() here would also delete
            // danmaku scheduled at/after `start`, losing them permanently.
            items = items.subList(start, items.size).toMutableList()
