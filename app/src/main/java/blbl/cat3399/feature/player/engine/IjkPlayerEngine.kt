...
internal class IjkPlayerEngine(
    ...
    // ijk native callbacks (OnPrepared/OnError/OnInfo/...) are dispatched on the player's
    // internal thread, not the main thread. All listener notifications must be hopped back to
    // the main thread, otherwise the UI code in PlayerActivity crashes with
    ...
    override fun setSource(source: PlaybackSource) {
        ...
        runCatching { ensurePlayer() }.onFailure { t ->
            mainHandler.post { listeners.forEach { it.onPlayerError(t) } }
            return
        }
    ...
    private fun updateState(state: Int) {
        ...
        mainHandler.post { listeners.forEach { it.onPlaybackStateChanged(state) } }
        notifyIsPlayingIfChanged()
    }
    ...
    private fun notifyIsPlayingIfChanged() {
        ...
        mainHandler.post { listeners.forEach { it.onIsPlayingChanged(now) } }
    }
        ...
        ijk =
            ...
            p.setOnErrorListener(
                IMediaPlayer.OnErrorListener { _, what, extra ->
                    ...
                    clearVisibleSeekPosition()
                    updateState(Player.STATE_IDLE)
                    mainHandler.post { listeners.forEach { it.onPlayerError(e) } }
                    true
                },
            )
            p.setOnInfoListener(
                IMediaPlayer.OnInfoListener { _, what, _ ->
                    when (what) {
                        ...
                        IMediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START -> {
                            ...
                            mainHandler.post { listeners.forEach { it.onRenderedFirstFrame() } }
                        }
                        ...
                        -> {
                            clearVisibleSeekPosition()
                            mainHandler.post { listeners.forEach { it.onPositionDiscontinuity(currentPosition) } }
                            settleSeekIfReady(p)
                        }
                    }
                    false
            ...
            p.setOnVideoSizeChangedListener(
                IMediaPlayer.OnVideoSizeChangedListener { _, width, height, _, _ ->
                    ...
                    mainHandler.post { listeners.forEach { it.onVideoSizeChanged(w, h) } }
                },
            )
