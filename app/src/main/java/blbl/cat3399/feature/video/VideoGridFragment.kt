...
class VideoGridFragment : Fragment(), RefreshKeyHandler, TabSwitchFocusTarget {
        ...
        if (!::adapter.isInitialized) {
            ...
            adapter =
                VideoCardAdapter(
                    ...
                    showWatchedBadge = BiliClient.prefs::watchedBadgeOtherLists,
                )
        }
                ...
                callbacks =
                    object : DpadGridController.Callbacks {
                        ...
                        override fun canLoadMore(): Boolean {
                            val s = paging.snapshot()
                            // Also guard against isLoading: otherwise a rapid down-press while a
                            // load is already in flight parks focus and then never releases it.
                            return !s.isLoading && !s.endReached
                        }
    ...
    private fun loadNextPage(isRefresh: Boolean = false) {
        ...
        if (startSnap.isLoading || startSnap.endReached) {
            // Already loading or at the end. If a DpadGridController down-press parked focus for
            // load-more, release it here, otherwise the user gets stuck on an unfocused container.
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                ...
                val applied = result.appliedOrNull() ?: run {
                    // Load was skipped/aborted (e.g. superseded by a newer generation). The
                    // DpadGridController may have parked focus for load-more; release it now so the
                    // user is not frozen on an unfocused RecyclerView.
                    dpadGridController?.consumePendingFocusAfterLoadMore()
                    return@launch
                }
                ...
                // Load failed: release any focus park so the user can keep navigating.
                dpadGridController?.consumePendingFocusAfterLoadMore()
            } finally {
