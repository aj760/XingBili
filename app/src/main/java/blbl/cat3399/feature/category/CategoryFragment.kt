...
class CategoryFragment : Fragment(), VideoGridTabSwitchFocusHost, BackPressHandler {
        ...
        pageCallback =
            object : androidx.viewpager2.widget.ViewPager2.OnPageChangeCallback() {
                override fun onPageSelected(position: Int) {
                    ...
                    if (zone != null) BiliClient.prefs.lastCategoryZoneKey = CategoryZones.stableKeyFor(zone)
    ...
    private fun setZones(nextZones: List<Zone>, force: Boolean = false) {
        ...
        // On a fresh open there is no current key yet: resume at the last visited zone instead of
        // always snapping back to the first tab ("全站"). Falls back to the first tab if the saved
        // zone is no longer part of the visible set.
        val restoreKey = currentKey ?: BiliClient.prefs.lastCategoryZoneKey?.takeIf { key ->
            zones.any { CategoryZones.stableKeyFor(it) == key }
        }
        mediator?.detach()
        mediator = null
        zones = next
        ...
        val targetIndex = restoreKey?.let { key -> zones.indexOfFirst { CategoryZones.stableKeyFor(it) == key }.takeIf { it >= 0 } } ?: 0
