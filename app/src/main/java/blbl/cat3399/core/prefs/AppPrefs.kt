...
class AppPrefs(context: Context) {
    ...
    // Last category zone the user navigated to, so re-entering the category page resumes there
    // instead of always starting at the first tab ("全站"). Null when never set / cleared.
    var lastCategoryZoneKey: String?
        get() = prefs.getString(KEY_LAST_CATEGORY_ZONE_KEY, null)?.trim()?.takeIf { it.isNotBlank() }
        set(value) {
            if (value.isNullOrBlank()) {
                prefs.edit().remove(KEY_LAST_CATEGORY_ZONE_KEY).apply()
            } else {
                prefs.edit().putString(KEY_LAST_CATEGORY_ZONE_KEY, value).apply()
            }
        }
    ...
    /**
     * D-Pad focus hint inside the player (TV). When a player control gains focus, a small
     * rounded bubble appears beneath it showing the button label so the user does not have to
     * press each one to figure out its function.
     */
    var focusHintEnabled: Boolean
        get() = prefs.getBoolean(KEY_FOCUS_HINT_ENABLED, true)
        set(value) = prefs.edit().putBoolean(KEY_FOCUS_HINT_ENABLED, value).apply()

    /**
     * Locally tracked BV IDs the user has finished watching (most recent first).
     * Used by video lists (notably the UP master space) to show a "已看过" badge so the user
     * can spot the next unwatched video without remembering its title. Capped at
     * [MAX_WATCHED_BVIDS] (FIFO) via [markWatched]. Survives reinstall? No — lives in prefs.
     */
    var watchedBvids: List<String>
        get() = loadStringList(KEY_WATCHED_BVIDS)
        set(value) = saveStringList(KEY_WATCHED_BVIDS, value)

    /** Show "已看过" badge on the UP master space video list. */
    var watchedBadgeUpMasterSpace: Boolean
        get() = prefs.getBoolean(KEY_WATCHED_BADGE_UP_MASTER, true)
        set(value) = prefs.edit().putBoolean(KEY_WATCHED_BADGE_UP_MASTER, value).apply()

    /** Show "已看过" badge on other video lists (search results, recommendations, etc.). */
    var watchedBadgeOtherLists: Boolean
        get() = prefs.getBoolean(KEY_WATCHED_BADGE_OTHER_LISTS, true)
        set(value) = prefs.edit().putBoolean(KEY_WATCHED_BADGE_OTHER_LISTS, value).apply()
    ...
    /**
     * Append a BV id to the front of [watchedBvids] (most recent first). Caps the list at
     * [MAX_WATCHED_BVIDS] by dropping the oldest entries. No-op when the id is already present
     * or blank.
     */
    fun markWatched(bvid: String) {
        val v = bvid.trim()
        if (v.isBlank()) return
        val current = watchedBvids
        if (v in current) return
        val capacity = MAX_WATCHED_BVIDS
        val out = ArrayList<String>(minOf(current.size + 1, capacity))
        out.add(v)
        for (s in current) {
            if (s == v) continue
            out.add(s)
    ...
    /** Drop all locally-recorded watched BV ids. */
    fun clearWatchedHistory() {
        prefs.edit().remove(KEY_WATCHED_BVIDS).apply()
    }
    ...
    companion object {
        ...
        private const val KEY_LAST_CATEGORY_ZONE_KEY = "last_category_zone_key"
        ...
        private const val KEY_FOCUS_HINT_ENABLED = "focus_hint_enabled"
        private const val KEY_WATCHED_BVIDS = "watched_bvids"
        private const val KEY_WATCHED_BADGE_UP_MASTER = "watched_badge_up_master_space"
        private const val KEY_WATCHED_BADGE_OTHER_LISTS = "watched_badge_other_lists"
        const val MAX_WATCHED_BVIDS = 500
