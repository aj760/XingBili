...
object ApkUpdater {
    const val ONLINE_UPDATE_DISABLED = false
    // 自托管更新源基址：应用会从此处拉取 CHANGELOG.md 与 blbl-<版本>-<channel>.apk
    private const val UPDATE_BASE_URL = "https://blbl-update.pages.dev"
    private const val DEBUG_APK_URL = "$UPDATE_BASE_URL/blbl-latest-debug.apk"
    private const val RELEASE_APK_URL = "$UPDATE_BASE_URL/blbl-latest-release.apk"
    private const val CHANGELOG_URL = "$UPDATE_BASE_URL/CHANGELOG.md"
    ...
    fun apkUrlFor(versionName: String): String {
        ...
        return "$UPDATE_BASE_URL/blbl-$cleanVersion-$channel.apk"
    }
