...
class CookieStore(
    ...
    override fun saveFromResponse(url: HttpUrl, cookies: List<Cookie>) {
        ...
        for (cookie in cookies) {
            ...
            // ConcurrentHashMap.compute performs the read-modify-write atomically per domain,
            // preventing two parallel API calls (each on its own coroutine) from clobbering
            // each other's cookies for the same host.
            store.compute(key) { _, old ->
                val list = (old ?: mutableListOf()).toMutableList()
    ...
    fun upsertAll(cookies: List<Cookie>) {
        if (cookies.isEmpty()) return
        // Group by domain so each compute() performs one atomic batch update per host.
        val byDomain = cookies.groupBy { it.domain }
        for ((key, domainCookies) in byDomain) {
            store.compute(key) { _, old ->
                val list = (old ?: mutableListOf()).toMutableList()
