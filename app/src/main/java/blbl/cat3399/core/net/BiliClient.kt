...
object BiliClient {
    ...
    private fun clientFor(url: String, noCookies: Boolean = false): OkHttpClient {
        ...
        // NOTE: the original `.let` was only bound to the `apiOkHttp` branch because of Kotlin's
        // operator precedence, so CDN hosts ignored `noCookies` and could leak SESSDATA to the
        // bilivideo CDN. Wrap the whole if-expression so `noCookies` is honoured for every host.
        return (if (
            ...
            .let { if (noCookies) apiOkHttpNoCookies else it }
    }
